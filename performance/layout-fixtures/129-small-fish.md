...
Keep the document in source order. Related explanations may share a row when each has a readable measure. The column width stays equal even when one explanation has more words than its neighbor.

### Workspace

Keep the reader's position while the window changes size. A narrow window stacks these explanations; a wider window lets them share a row with one consistent gutter.
