    ...
    pub(crate) fn hit_inner(
        ...
        if self.flags.is_inline_root() {
            ...
            if let Some(ild) = element_data.inline_layout_data.as_ref() {
                ...
                {
                    // Ligature continuations have an advance and source text,
                    // but no glyph of their own. Resolve their owning style
                    // without aborting hit testing on a glyphless cluster.
                    let node_id = cluster.first_style().brush.id;
