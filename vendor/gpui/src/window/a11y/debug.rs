...
pub(crate) struct A11yDebug {
    enabled: bool,
    last_tree_update: super::snapshot::RetainedTreeSnapshot,
...
impl Default for A11yDebug {
    fn default() -> Self {
        Self {
            // Provenance is always useful in developer builds. Optimized
            // applications opt in explicitly so activating assistive
            // technology does not retain another complete large-document
            // tree solely for a debug dump that was never requested.
            enabled: cfg!(debug_assertions) || std::env::var_os("GPUI_A11Y_DEBUG").is_some(),
            last_tree_update: Default::default(),
            last_gpui_focus: None,
            last_active_descendant: None,
            frame_number: 0,
            last_frame: None,
            #[cfg(debug_assertions)]
            last_node_info: Default::default(),
        }
    }
}
...
impl A11yDebug {
    pub(crate) fn capture(
        ...
        self.last_tree_update.capture(update);
        let node_count = self
            .last_tree_update
            .update()
            .map_or(update.nodes.len(), |snapshot| snapshot.nodes.len());
    ...
    pub(crate) fn to_json(&self) -> Option<String> {
        let update = self.last_tree_update.update()?;
