                    ...
                    wl_keyboard::KeyState::Pressed if !keysym.is_modifier_key() => {
                        ...
                        if let Some(mut compose) = state.compose_state.take() {
                            ...
                            match compose.status() {
                                ...
                                xkb::Status::Cancelled if keysym == xkb::Keysym::Escape => {
                                    // Escape is cancellation, not an invalid printable
                                    // compose sequence. Leave the marked text owned by
                                    // the focused editor until the key event below lets
                                    // it restore its pre-composition text and selection.
                                    // Inserting the accent here would commit that edit
                                    // before Escape could cancel it.
                                    state.pre_edit_text.take();
                                    compose.reset();
                                }
