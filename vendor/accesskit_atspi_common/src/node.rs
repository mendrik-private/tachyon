...
impl NodeWrapper<'_> {
    ...
    fn attributes(&self) -> HashMap<&'static str, String> {
        ...
        // Orca reconstructs MathML using each node's standard `tag` object
        // attribute. The same mapping also preserves ordinary HTML semantics.
        if let Some(tag) = self.0.data().html_tag() {
            attributes.insert("tag", tag.to_owned());
        }
...
mod tachyon_tests {
    ...
    #[test]
    fn mathml_tag_reaches_native_object_attributes() {
    ...
    #[test]
    fn expansion_maps_absence_collapsed_and_expanded_distinctly() {
        for expanded in [None, Some(false), Some(true)] {
            let id = LocalNodeId(1);
            let mut data = accesskit::Node::new(Role::Button);
            data.set_label("Disclosure");
            data.add_action(Action::Click);
