...
pub use a11y::{A11ySubtreeBuilder, RetainedA11ySubtree, RetainedA11ySubtreeError};
...
impl Window {
    ...
    /// Register a frame-local handler for actions on a synthetic subtree.
    /// Node-specific listeners run first. Return true only for a target/action
    /// owned by this handler; false preserves subsequent handlers and built-ins.
    /// Register during paint, like `on_a11y_action`; registration expires next frame.
    pub fn on_a11y_action_request(
        &mut self,
        listener: impl FnMut(&accesskit::ActionRequest, &mut Window, &mut App) -> bool + 'static,
    ) {
        self.a11y.action_request_listeners.push(Box::new(listener));
    }
    ...
    pub(crate) fn handle_a11y_action(&mut self, request: accesskit::ActionRequest, cx: &mut App) {
        ...
        let mut listeners = std::mem::take(&mut self.a11y.action_request_listeners);
        let handled = listeners
            .iter_mut()
            .any(|listener| listener(&request, self, cx));
        self.a11y.action_request_listeners = listeners;
        if handled {
            return;
        }
